...
- [x] T6 — `telegram-streamer` (1184 lineas) — CERRADO 2026-08-30, commit eb999a4, 61 tests. Verificado: 61/61, y CERO bytes cambiados en src/. HALLAZGO DE FONDO: el modulo no tiene module.exports y al requerirlo YA lee credenciales, arranca setInterval y puede POSTear a api.telegram.org. Solucion sin tocar el fuente: un loader con vm que compila solo lo anterior al marcador `// --- Startup ---`. Cubre el 100% de la logica de decision (rate-limit, dedup, truncado, eviction) y ~57% de las funciones, con lo no cubierto documentado. owns=src/telegram-streamer.cjs,test/telegram-streamer.test.cjs
...
- [~] T11 — `github-webhook` (202) — EN CURSO, con HALLAZGO DE SEGURIDAD elevado al operador el 2026-08-30. `verifySignature` hace FAIL-OPEN: linea 36 `process.env.GITHUB_WEBHOOK_SECRET || ''` y linea 49 `if (!SECRET) return true`. Dos agravantes medidos: (1) el `|| ''` colapsa 'sin definir' con 'definida vacia' — un typo de env-file desactiva la verificacion sin ninguna senal en runtime; (2) `server.listen(PORT, ...)` en la 195 no pasa host, y la 196 LOGUEA `http://0.0.0.0:PORT` — el modulo anuncia que bindea todas las interfaces. El comentario del header dice 'usar solo en redes privadas', pero un webhook de GitHub tiene que ser alcanzable desde internet POR DEFINICION, asi que esa condicion la excluye el proposito del modulo. EL CONTRASTE QUE LO CIERRA: `dashboard-server-routes.cjs:312` hace `listen(PORT, BIND_HOST, ...)` — el patron correcto vive en el mismo repo. MEDIDO: el proceso NO corre en esta maquina y la variable no esta definida; podria correr en otro host, sin verificar. NO SE ARREGLA sin decision del operador: pasar fail-open a fail-closed rompe cualquier deployment que hoy corra sin secret. owns=src/github-webhook.cjs,test/github-webhook.test.cjs
...
- [ ] T26 — la suite NO SE PUEDE CORRER desde un worktree: varios tests resuelven rutas relativas como `../../_docs-curation`, que asumen la profundidad del checkout principal y se rompen un nivel mas abajo bajo `_worktrees/`. Medido el 2026-08-30 por DOS agentes independientes que no se coordinaron: T4 conto 41 fails y T6 entre 51 y 70, todos ajenos a su cambio y fluctuantes entre corridas. IMPORTA porque los worktrees son exactamente donde trabajan los agentes: hoy ninguno puede medir su propia linea base, y un agente sin linea base confiable tiene todos los incentivos para leer un fallo ambiental como regresion propia — o peor, para debilitar un test sano hasta que pase. Arreglo: resolver esas rutas desde el root del repo (`git rev-parse --show-toplevel`, o una constante calculada una vez) en vez del cwd del test. owns=test/
