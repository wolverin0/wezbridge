...
- [ ] T28 — tres lugares mas con la profundidad hardcodeada, dejados FUERA de T26 a proposito. `scripts/fleet-directory.cjs` y `scripts/fleet-status.cjs` tienen el mismo defecto que rotate-pane —sin escape hatch de WEZBRIDGE_INTEL_DIR— y hoy no los cubre ningun test, asi que el bug esta latente y nadie se enteraria. `scripts/orchestrator-turn.cjs:521` es el cwd de un spawn headless REAL de claude: el agente NO lo toco y dijo por que, que es la decision correcta — cambiar el cwd de un spawn real sin un test que lo respalde es mas riesgoso que el bug que arregla. Ademas hay 12 modulos con la misma forma que caen a WEZBRIDGE_INTEL_DIR primero, asi que solo disparan si esa variable falta: riesgo bajo, no cero. La primitiva ya existe — `src/repo-root.cjs`, creado en T26. CRITERIO: los dos scripts sin escape hatch resuelven bien desde un worktree, con un test que hoy sea ROJO desde `_worktrees/`; y para el spawn cwd, PRIMERO un test que lo cubra y recien despues el cambio. owns=scripts/fleet-directory.cjs,scripts/fleet-status.cjs,test/fleet-scripts-repo-root.test.cjs
