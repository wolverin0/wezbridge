...
| `scripts/daemon-heartbeat-sentinel.cjs:142-165` (`deliverPoke`, PRE-fixup) — **finding del verificador sobre PR #58, faltaba de este inventario original** | El poke `[daemon-sentinel] DAEMON DOWN/WEDGED/HTTP UNRESPONSIVE ...` de la tarea programada de Windows `WezBridge-DaemonSentinel` (cada 5 min) | El pane del proyecto `wezbridge` (`WEZBRIDGE_ORCH_REPO`) | `findOrchestratorPane()` — `pane-discovery.cjs`'s `discoverPanes()` filtrado por `isClaude` + basename de proyecto, sin flag, sin fallback Orca | (a) mensajeria de flota — DEBE ir por Orca. Corregido en este fixup: `deliverPoke` ahora despacha a `deliverPokeOrca` (default, `src/orca-target.cjs` + `src/orca-send.cjs`, mismo patron que `attemptSendOrca` de decision-relay, self-send guard vía `ORCA_TERMINAL_HANDLE`) o `deliverPokeWezTerm` (legado, detrás de `WEZBRIDGE_WEZTERM_TRANSPORT=1`). Dedupe/cooldown/deadman de `evaluate()`/`main()` sin tocar. Ver `test/daemon-heartbeat-sentinel-orca.test.cjs`. |
