...
function render(rows, { liveKnown, generatedAt }) {
  ...
  L.push('<!-- doc-head: Generated fleet routing and executable rules (T-0327) -->');
  L.push('Read for ownership, routing, A2A 900-character limits, cross-repo provenance and routine run evidence.');
  L.push('Generated by wezbridge/scripts/fleet-directory.cjs; runtime readiness requires separate verification.');
  L.push('<!-- /doc-head -->');
  L.push('');
  ...
  L.push('');
  L.push('## Reglas de flota');
  L.push('');
  L.push('| Regla | Guard ejecutable |');
  L.push('|---|---|');
  L.push('| Sobres A2A de hasta 900 caracteres; los mayores se rehusan y remiten a `_intel/briefs/`. Solo `allow_long: true` permite inline largo, conservando byte cap y criteria. | `wezbridge/src/a2a-length-guard.cjs` y el handler `a2a_send` en `wezbridge/src/mcp-server.cjs` |');
  L.push('| Trabajo cross-repo en `fix/*`: tarjeta del repo destino y corr exacto. Registrar procedencia al comenzar con `node wezbridge/scripts/cross-repo-audit.cjs record --from <origen> --repo <destino> --cwd <worktree> --corr <corr> --task <id>`. Rama y draft PR no autorizan merge. | `wezbridge/scripts/cross-repo-audit.cjs` lee `actions.jsonl`; `fleet-steward.cjs` emite `cross-repo-unticketed`, gate de 24h sin reiniciar por actividad repetida. No atribuye ramas no registradas. |');
  L.push('| Todo bot periodico se registra con tarea y cadencia, y deja `run-*.json` en `_intel/routine-findings`. Exit 0 de schtasks no acredita el trabajo. | `wezbridge/scripts/routine-registry.cjs` y `routine-audit.cjs`: registro vencido sin run en su ventana produce `routine-void`; un JSON roto queda como void local. |');
  L.push('');
  L.push('Formato de registros, ventanas y controles: `wezbridge/docs/a2a-protocol.md`, seccion Reglas de flota.');
  L.push('');
  L.push('## The default directory is usually NOT where the work is');
  L.push('');
  L.push('Measured 2026-07-29 across four repos in one afternoon: `mutual` has three worktrees and its');
  L.push('executor works in `_worktrees/mutual-battle-test` while `Py Apps/mutual` sat 31 commits behind');
  L.push('on another branch; `pedrito` runs its canary in `G:/tmp/pedrito-branch-bridge` and its prompt');
  L.push('explicitly forbids touching the OneDrive checkout; `clawtrol` cut a fresh worktree because the');
  L.push('named tree was dirty with operator files; `mzcopilot` deployed from');
  L.push('`_worktrees/mzcopilot-megascript-rebuild` while its primary checkout is 28 behind and 1 ahead.');
  L.push('');
  L.push('So `git worktree list` BEFORE comparing any sha, and require reports to name the worktree in');
  L.push('their evidence. Reading a primary checkout produced two confident, false findings in one day —');
  L.push('a branch-integrity mismatch that did not exist, and a scheduled dispatch judged wrong when it');
  L.push('was right. A stale tree answers every question fluently and incorrectly.');
  L.push('');
